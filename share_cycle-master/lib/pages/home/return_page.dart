...
class _ReturnPageState extends State<ReturnPage> {
  ...
  getGoodsApply() async {
    ...
    var applyResult = await db
        ...
        .where("state", whereIn: [3, 4]) // 2 rejected  3 borrowed  4 returned
        .get();
    for (var value in applyResult.docs) {
      dataApply.add({
        ...
        "lend_id": value["lend_id"], // id
        "applicant_user_id": value["applicant_user_id"], // applicant id
        "lend_user_id": value["lend_user_id"], // lender id
        "state": value["state"], // 0 Just Submitted, 1 Approved, 2 Rejected
        "borrowing_date": value["borrowing_date"] ?? "" // borrow time
      });
    }
    setState(() {});
  }
