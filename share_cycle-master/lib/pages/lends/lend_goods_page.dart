...
class LendGoodsPage extends StatefulWidget {
  ...
  LendGoodsPage({super.key, required this.goodsType, required this.type, this.oldData});
...
class _LendGoodsPageState extends State<LendGoodsPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      if (widget.oldData != null) {
        ...
        oldImageUrl = (json.decode(widget.oldData["goods_img"]) as List<dynamic>).map((e) => e).toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: titleWidget(
        title: "${S.of(context).upload_goods}",
        leading: true,
      ),
      body: Container(
                      ...
                      : PageView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            return Image.network(
                              ImageUtils.firebaseImageUrl(fileName: oldImageUrl[index]),
                              fit: BoxFit.fill,
                              width: double.maxFinite,
                            );
                          },
              ...
              Row(
                children: [
                  Expanded(
                    ...
                    onTap: () {
                      showDateSelect(context, startDate: DateTool.getToDay(), endDate: "", selectDate: (value) {
                        setState(() {
                          startDate = value;
                        });
                      });
                  ...
                  Expanded(
                    ...
                    onTap: () {
                      if(startDate==""){
                        SnackUtils.showSnack(context, "${S.of(context).please_select_start_time_lend}");
                        return;
                      }
                      showDateSelect(context, startDate: startDate, endDate: "", selectDate: (value) {
                        setState(() {
                          endDate = value;
                        });
                      });
                ...
                Container(
                  ...
                  child: ElevatedButton(
                      onPressed: () async {
                        ...
                        if (goodsImage == "") {
                          SnackUtils.showSnack(context, "${S.of(context).please_select_an_item_photo}"); //请选择物品照片  Please select an item photo
                          return;
                        }
                        if (name == "") {
                          SnackUtils.showSnack(context, "${S.of(context).please_fill_in_the_name_of_the_lender}"); //请填写出借人名称
                          return;
                        }
                        if (goodsName == "") {
                          SnackUtils.showSnack(context, "${S.of(context).please_fill_in_the_item_name}"); //请填写物品名称
                          return;
                        }
                        ...
                        // }
                        if (startDate == "") {
                          SnackUtils.showSnack(context, "${S.of(context).please_select_the_start_date}");
                          return;
                        }
                        if (endDate == "") {
                          SnackUtils.showSnack(context, "${S.of(context).please_select_the_end_date}");
                          return;
                        }
                        if (information == "") {
                          SnackUtils.showSnack(context, "${S.of(context).please_fill_in_the_description_information}"); // Please fill in the description information
                          return;
                        }
                        ...
                        final lend = <String, dynamic>{
                          ...
                          "goods_img": goodsImage,  // 如果没有修改，则使用之前的部分
                          ...
                          "state": 0, // 0 是未借出  1 是已经借出
                        };
                        // 判断是修改还是新增
                        if (widget.oldData != null) {
                          // 进行修改
                          await db.collection('lend').doc(widget.oldData['id']).update(lend);
                          SnackUtils.showSnack(context, "${S.of(context).successfully_update_the_loaned_item}"); //修改出借物品添加成功
                          setState(() {
                            loading = false;
                          });
                        } else {
                          var createValue = await db.collection('lend').add(lend);
                          if (createValue.id != "") {
                            SnackUtils.showSnack(context, "${S.of(context).successfully_added_the_loaned_item}"); //出借物品添加成功
                          }
                          setState(() {
                            loading = false;
                          });
