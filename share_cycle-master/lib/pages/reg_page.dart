...
class _RegPageState extends State<RegPage> {
  ...
  regUser() async {
    ...
    //修改验证的状态
    await db.collection("email").doc(queryDocumentSnapshot.id).update({"state":1,});
  ...
  sendWR(String email) async {
    ...
    if (!EmailUtils.checkEmail(context, email,format: "@qq.com")) {
      return;
    }
    ...
    if (getUser.size > 0) {
      // SnackUtils.showSnack(context, "The email already exists");
      SnackUtils.showSnack(
          context, "${S.of(context).the_email_already_exists}"); // 已经注册过
      setState(() {
        loading = false;
      });
      return;
    ...
    int min = 1000; // 最小值（4位数的最小值是1000）
    int max = 9999; // 最大值（4位数的最大值是9999）
    int verificationCode = min + random.nextInt(max - min); //验证码
    // 在数据存储一份验证码
    await db
        .collection("email")
